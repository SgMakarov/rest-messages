...
class Message(models.Model):
    """
    Django model for a Message.
    As there is no login, I decided not to create a model for the user and just 
    store data in message. For the real project I'd created this model, 
    reference it with some foreign key, but for CRUD operations on message it is
    redundant.
